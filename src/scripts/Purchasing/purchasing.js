...
const Purchasing = {
    // Tools available in the purchasing module
    purchasingTools: [
        {
            id: 'price-lookup',
            name: 'Price Lookup',
    ...
    // Render the purchasing tool cards
    renderToolCards: function() {
        ...
        cardContainer.className = 'purchasing-tools-container';
        
        // Create cards for each purchasing tool
        this.purchasingTools.forEach(tool => {
            const card = document.createElement('div');
            card.className = 'purchasing-tool-card';
    ...
    // Setup event listeners for the purchasing tools
    setupEventListeners: function() {
        const container = document.querySelector('.purchasing-content');
        if (!container) return;
        
        container.addEventListener('click', (event) => {
            const toolCard = event.target.closest('.purchasing-tool-card');
            if (toolCard) {
                const toolId = toolCard.getAttribute('data-tool');
                const tool = this.purchasingTools.find(t => t.id === toolId);
    ...
    // Load a specific purchasing tool
    loadTool: function(tool) {
        console.log(`Loading purchasing tool: ${tool.name}`);
